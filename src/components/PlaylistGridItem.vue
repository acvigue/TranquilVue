<template>
  <button
    :style="style"
    class="group flex flex-col items-center p-4 rounded-xl bg-gray-700 gap-3 hover:bg-gray-600 active:scale-90 transition transform-gpu duration-300 relative"
  >
    <PatternPreview
      :pattern="f_p"
      class="h-44 w-44 rounded-full border-gray-500 border-[3px] bg-gray-800 group-hover:scale-105 transition transform-gpu duration-300"
      lineColor="#ffffff"
      :showBall="false"
    />
    <span
      class="w-[90%] text-center text-md font-medium line-clamp-1 text-ellipsis overflow-hidden break-words"
    >
      {{ item.name }}</span
    >
  </button>
</template>

<script setup lang="ts">
import { type Playlist, type Pattern } from '@/stores/files'
import { onMounted, type StyleValue, ref } from 'vue'
import useFilesStore from '../stores/files'
import PatternPreview from '../components/PatternPreview.vue'
import tranquilapi from '../plugins/tranquilapi'

const files = useFilesStore()

interface PlaylistGridItemPlaceholderProps {
  style?: StyleValue
  item: Playlist
}

let featured_pattern;
let f_p = ref({} as Pattern)

onMounted(async () => {
  featured_pattern = files.getPattern(props.item.featured_pattern);
  if(!featured_pattern) {
    featured_pattern = (await tranquilapi.get(`/patterns/${props.item.featured_pattern}`)).data
  }
  f_p.value = featured_pattern
})

const props = defineProps<PlaylistGridItemPlaceholderProps>()
</script>
